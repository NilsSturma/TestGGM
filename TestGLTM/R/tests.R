...
test_symmetric <- function(X, ind_eq, ind_ineq1, ind_ineq2, E=1000, alphas=seq(0.01, 0.99, 0.01)){
  ...
  # create permutations
  perm = matrix(unlist(combinat::permn(1:4)), ncol=4, byrow=TRUE)
...
# Y_centered = Rfast::transpose(Rfast::transpose(Y) - Y_mean)
# for (i in 1:E) {
#   batchsum = matrix(0, nrow=omega, ncol=p)
#   for (j in 1:p){
#     for (b in 1:omega){
#       L = seq(1 + (b-1)*B, (b)*B);
#       for (l in 1:B){
#         batchsum[b,j] = batchsum[b,j] + Y_centered[L[l],j];
#       }
#     }
#   }
# }
